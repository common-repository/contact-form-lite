...
.most_popular {top: -5px;position: absolute;width: 115px;height: 85px;background: transparent url("<?php echo esc_url( ECF_URL );?>/inc/images/most_popular.png") no-repeat scroll left top;right: -5px;}

.ecf-container-cnt .plugin-card-top .column-description {
	margin-left: 140px !important;
}

body.rtl .ecf-container-cnt .plugin-card-top .column-description {
	...
	<div id="the-list">
		...
		<div id="<?php echo esc_attr( $plugin["slug"] ); ?>" class="plugin-card drop-shadow lifted">
            ...
            <?php if ( isset( $plugin["slug"] ) && $plugin["slug"] == 'easy-media-gallery' ) {echo '<div class="most_popular"></div>';} ?>
				<a href="<?php echo esc_url( $details_link ); ?>" class="ecf-free-plgn thickbox plugin-icon"><img width="128" height="128" src="<?php echo esc_attr( $plugin_icon_url ) ?>" /></a>
				...
				<div class="desc column-description" style="margin-right: 20px !important;">
					...
					<?php
						if ( $action_links ) {
							...
							echo '<li>' . wp_kses( $action_links[0], ecf_wp_kses_allowed_html() ) . '</li>';
			...
			<div class="plugin-card-bottom">
				<div class="column-updated">
					<strong><?php esc_html_e( 'Last Updated:' ); ?></strong> <span title="<?php echo esc_attr( $plugin['last_updated'] ); ?>">
						<?php printf( '%s ago', esc_html( human_time_diff( strtotime( $plugin['last_updated'] ) ) ) ); ?>
					</span>
				</div>
<div class="column-downloaded">
				...
				<div class="column-compatibility">
					...
					if ( ! empty( $plugin['tested'] ) && version_compare( substr( $GLOBALS['wp_version'], 0, strlen( $plugin['tested'] ) ), $plugin['tested'], '>' ) ) {
						echo '<span class="compatibility-untested">' . esc_html( 'Untested with your version of WordPress' ) . '</span>';
